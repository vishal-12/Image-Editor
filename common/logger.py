    ...
    def exit_log():
        ...
        try:
            ...
            if log_json_out:
                ...
                r = requests.post(log_json_out, json=logs, timeout=60 )
            ...
            logging.info("Error in exit_log: [%s]" % e.message )
            print (json.dumps(logs))
        sys.exit(0)
    ...
    def send_inline_logs(inlinelogs):
        ...
        try:
            if inline_callback_url:
                ...
                r = requests.post(inline_callback_url, json=inlinelogs, timeout=60 )
                if r.status_code != 200:
                    logging.info("Logger API response error status code: %s" % r.status_code )
                    logging.info(r.text)
                else:
            ...
            logging.info("Error in Send Inline Logs: [%s]" % e.message )
